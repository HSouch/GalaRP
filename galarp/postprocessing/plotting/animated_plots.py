...
def animated_hexbin_plot(orbits, x_ind=1, y_ind=2, n_frames=100, **kwargs):
    ...
    wind_profile = np.sqrt(np.sum(orbits.metadata["WIND"].evaluate(orbits.data.t) ** 2, axis=1)) * u.kpc/u.Myr

    dens_profile = orbits.metadata["RHO_ICM"].evaluate(orbits.data.t)
